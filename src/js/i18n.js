class I18n {
  ...
  setupLanguageSwitcher() {
    if (!document.getElementById("language-switcher")) {
      ...
      switcher.className =
        "fixed bottom-6 left-1/2 -translate-x-1/2 md:left-auto md:right-6 md:translate-x-0 z-50";

      switcher.innerHTML = `
      <div class="flex items-center gap-1 bg-slate-900/80 backdrop-blur-md border border-white/10 p-1.5 rounded-full shadow-2xl">
        <button data-lang="es" class="flex items-center justify-center w-12 h-10 md:w-10 md:h-8 rounded-full text-xs font-bold transition-all duration-200 ${
          this.locale === "es"
        ...
        <button data-lang="en" class="flex items-center justify-center w-12 h-10 md:w-10 md:h-8 rounded-full text-xs font-bold transition-all duration-200 ${
          this.locale === "en"
            ? "bg-blue-500 text-white shadow-lg"
            : "text-slate-300 hover:text-white"
        }">
